 ...
 * 
 * Currently, it only checks to see if the methods are of the correct length for
 * Java methods (&lt;64K).
 * 
...
public class VerifyingClassAdapter extends ClassVisitor {
    private static final Logger logger = Logger
            .getLogger(VerifyingClassAdapter.class.getName());
    ...
    /**
     * @param cw
     *            A class writer that is wrapped by this class adapter
     * @param original
    ...
    public byte[] toByteArray() {
        if (state != State.PASS) {
            logger.log(Level.WARNING, "Failed to instrument class " + className
                    + " because " + message);
            return original;
        }
