...
public class AllocationRecorder {
    static {
        ...
        // JVM-crashing assert in JPLISAgent.c, so we make sure to not call it
        // after
        // shutdown. There can still be a race here, depending on the extent of
        // the
    ...
    private static final BoundedMap<Class<?>, Long> classSizesMap = new BoundedMap<Class<?>, Long>(
            131072, 0.75);
     ...
     *            the object.
     * @param isArray
     *            indicates if the given object is an array.
     ...
     * 
     * @param count
     *            the count of how many instances are being allocated, if an
     *            array is being allocated. If an array is not being allocated,
     *            then this value will be -1.
     * @param desc
     *            the descriptor of the class/primitive type being allocated.
     * @param newObj
     *            the new <code>Object</code> whose allocation is being
     *            recorded.
     */
    ...
    public static void recordAllocationForceForTestReal(int count, String desc,
            Object newObj, int recurse) {
        if (recurse != 0) {
