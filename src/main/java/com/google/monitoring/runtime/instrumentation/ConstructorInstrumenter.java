 ...
 * will want to add {@link ConstructorCallback}s by invoking
 * {@link #instrumentClass(Class, ConstructorCallback)}.
 * 
...
public class ConstructorInstrumenter implements ClassFileTransformer {
    ...
    private static final Logger logger = Logger
            .getLogger(ConstructorInstrumenter.class.getName());
    private static ConcurrentHashMap<Class<?>, List<ConstructorCallback<?>>> samplerMap = new ConcurrentHashMap<Class<?>, List<ConstructorCallback<?>>>();
     ...
     * @param sampler
     *            the code to be invoked when an instance of c is constructed
     */
    public static void instrumentClass(Class<?> c,
            ConstructorCallback<?> sampler) throws UnmodifiableClassException {
        ...
        // executed directly after the retransformClasses is called. Thus, we
        // need
     ...
     * 
     * @param originalBytes
     *            the original <code>byte[]</code> code.
    ...
    public static byte[] instrument(byte[] originalBytes,
        ...
        try {
            ...
            VerifyingClassAdapter vcw = new VerifyingClassAdapter(cw,
                    originalBytes, cr.getClassName());
            ClassVisitor adapter = new ConstructorClassAdapter(vcw,
                    classBeingRedefined);
    ...
    /**
     * The per-method transformations to make. Really only affects the <init>
     * methods.
     */
    static class ConstructorMethodAdapter extends MethodVisitor {
        /**
         * The LocalVariablesSorter used in this adapter. Lame that it's public
         * but the ASM architecture requires setting it from the outside after
         * this AllocationMethodAdapter is fully constructed and the
         * LocalVariablesSorter constructor requires a reference to this
         * adapter. The only setter of this should be
        ...
        public void visitInsn(int opcode) {
            ...
            if (opcode == Opcodes.RETURN) {
                ...
                super.visitMethodInsn(
                        ...
                        "invokeSamplers", "(Ljava/lang/Object;)V", false);
            }
    ...
    /**
     * Bytecode is rewritten to invoke this method; it calls the sampler for the
     * given class. Note that, unless the javaagent command line argument
    ...
    public static void invokeSamplers(Object o) {
        ...
        while (currentClass != null) {
            List<ConstructorCallback<?>> samplers = samplerMap
                    .get(currentClass);
            if (samplers != null) {
                for (ConstructorCallback sampler : samplers) {
                    sampler.sample(o);
                }
                // Return once the first list of registered samplers are found
                // and invoked.
                return;
            } else {
                // When subclassesAlso is not specified (default), return if no
                // samplers are registered with the type of the
     ...
     * the per-method transformer above if the method is an {@code <init>}
     * method.
     */
    static class ConstructorClassAdapter extends ClassVisitor {
         ...
         * <code>visitMethod</code> is called and the returned MethodVisitor is
         * used to visit the method. Note that a new MethodVisitor is
        ...
        public MethodVisitor visitMethod(int access, String name, String desc,
            ...
            if ((mv != null) && "<init>".equals(name)) {
                ConstructorMethodAdapter aimv = new ConstructorMethodAdapter(
                        mv, cl);
