...
public class AllocationInstrumenter implements ClassFileTransformer {
    static final Logger logger = Logger.getLogger(AllocationInstrumenter.class
            .getName());
    ...
    static boolean canRewriteClass(String className, ClassLoader loader) {
        ...
        // 2. If it is java.lang.ThreadLocal, which can't be rewritten because
        // the
    ...
    // No instantiating me except in premain() or in {@link
    // JarClassTransformer}.
    ...
    public static void premain(String agentArgs, Instrumentation inst) {
        ...
        List<String> args = Arrays.asList(agentArgs == null
                ? new String[0]
                : agentArgs.split(","));
        ...
        // instrumented.
        ConstructorInstrumenter.subclassesAlso = args
                .contains("subclassesAlso");
    ...
    /**
     * Given the bytes representing a class, go through all the bytecode in it
     * and instrument any occurences of new/newarray/anewarray/multianewarray
     * with pre- and post-allocation hooks. Even more fun, intercept calls to
     * the reflection API's Array.newInstance() and instrument those too.
     * 
     * @param originalBytes
     *            the original <code>byte[]</code> code.
     * @param recorderClass
     *            the <code>String</code> internal name of the class containing
     *            the recorder method to run.
     * @param recorderMethod
     *            the <code>String</code> name of the recorder method to run.
    ...
    public static byte[] instrument(byte[] originalBytes, String recorderClass,
        ...
        try {
            ...
            ClassVisitor adapter = new AllocationClassAdapter(vcw,
    ...
    /**
     * @see #instrument(byte[], String, String, ClassLoader) documentation for
     *      the 4-arg version. This is a convenience version that uses the
     *      recorder in this class.
     */
