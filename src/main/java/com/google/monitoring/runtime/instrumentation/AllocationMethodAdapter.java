 ...
 * to record the allocation being done for profiling. Instruments bytecodes that
 * allocate heap memory to call a recording hook.
 * 
 * @author Ami Fischman
 */
class AllocationMethodAdapter extends MethodVisitor {
     ...
     * here sometimes; one reason is toArray()-type methods that require an
     * array type argument (see ArrayList.toArray() for example).</li>
    ...
    /**
     * Like RECORDER_SIGNATURE, but for a method that extracts all of the
     * information dynamically from a class.
     */
    ...
    private static final String[] primitiveTypeNames = new String[]{"INVALID0",
            "INVALID1", "INVALID2", "INVALID3", "boolean", "char", "float",
     ...
     * constructor requires a reference to this adapter. The only setter of this
     * should be AllocationClassAdapter.visitMethod().
     */
    public LocalVariablesSorter lvs = null;

    /**
     * A new AllocationMethodAdapter is created for each method that gets
     * visited.
     */
    ...
    public void visitIntInsn(int opcode, int operand) {
        if (opcode == Opcodes.NEWARRAY) {
            ...
            if (operand >= 4 && operand <= 11) {
                ...
                AllocationInstrumenter.logger
                        .severe("NEWARRAY called with an invalid operand "
                                + operand
     ...
     * {@link java.lang.Object#clone} is also a call to INVOKESPECIAL, and is
     * hooked here. {@link java.lang.Class#newInstance} and
     * {@link java.lang.reflect.Constructor#newInstance} are both INVOKEVIRTUAL
     * calls, so they are hooked here, as well.
     */
    @Override
    public void visitMethodInsn(int opcode, String owner, String name,
        ...
        if (opcode == Opcodes.INVOKESTATIC
            ...
            if (signature.equals("(Ljava/lang/Class;I)Ljava/lang/Object;")) {
                ...
                // -> stack: ... class className
                int typeNameIndex = newLocal("Ljava/lang/String;",
                        beginScopeLabel, endScopeLabel);
                ...
                return;
                    ...
                    .equals("(Ljava/lang/Class;[I)Ljava/lang/Object;")) {
                Label beginScopeLabel = new Label();
                Label endScopeLabel = new Label();
                super.visitLabel(beginScopeLabel);

                int dimsArrayIndex = newLocal("[I", beginScopeLabel,
                        endScopeLabel);
        ...
        if (opcode == Opcodes.INVOKESPECIAL) {
            if ("clone".equals(name) && "java/lang/Object".equals(owner)) {
                ...
                // Tricky because superclass initializers mean there can be more
                // calls
                // to <init> than calls to NEW; hence outstandingAllocs.
                --outstandingAllocs;

                // Most of the time (i.e. in bytecode generated by javac) it is
                // the case
                // that following an <init> call the top of the stack has a
                // reference ot
                // the newly-initialized object. But nothing in the JVM Spec
                // requires
                // this, so we need to play games with the stack to make an
                // explicit
    ...
    private void buildRecorderFromObject(int opcode, String owner, String name,
            String signature) {
    ...
    /**
     * new and anewarray bytecodes take a String operand for the type of the
     * object or array element so we hook them here. Note that new doesn't
    ...
    public void visitTypeInsn(int opcode, String typeName) {
        if (opcode == Opcodes.NEW) {
            // We can't actually tag this object right after allocation because
            // it
    ...
    public void visitMaxs(int maxStack, int maxLocals) {
        if (localScopes != null) {
            for (VariableScope scope : localScopes) {
                super.visitLocalVariable("xxxxx$" + scope.index, scope.desc,
                        null, scope.start, scope.end, scope.index);
            }
        }
    ...
    private int newLocal(String typeDescriptor, Label begin, Label end) {
        return newLocal(Type.getType(typeDescriptor), typeDescriptor, begin,
                end);
    }
    ...
    void calculateArrayLengthAndDispatch(String typeName, int dimCount) {
        ...
        // time, we take the created multi-dimensional array and peel off
        // nesting
        // levels from the left. For each nesting layer we probe the array
        // length
        // and accumulate a partial product which we can then feed the recording
        // function.

        // below we note the partial product of dimensions 1 to X-1 as
        // productToX
        // (so productTo1 == 1 == no dimensions yet). We denote by aref0 the
        // array reference at the current nesting level (the containing aref's
        // [0]
        ...
        for (int i = 0; i < dimCount; ++i) {
            ...
            super.visitInsn(Opcodes.SWAP); // -> stack: ... origaref productToI
                                           // aref
            super.visitInsn(Opcodes.DUP_X1);
